...
func (c *Client) doRequest(method, path string, header http.Header, body io.Reader) (*Response, error) {
	return c.doBaseRequest(method, "/api/v1"+path, header, body)
}

func (c *Client) doBaseRequest(method, path string, header http.Header, body io.Reader) (*Response, error) {
	c.mutex.RLock()
	debug := c.debug
	if debug {
		var bodyStr string
...
func (c *Client) getWebResponseReader(method, path string, header http.Header, body io.Reader) (io.ReadCloser, *Response, error) {
	resp, err := c.doBaseRequest(method, path, header, body)
	if err != nil {
		return nil, resp, err
	}
