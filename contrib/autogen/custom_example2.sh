...
set_version() {
  ...
  # Read version elaborated by process and transform phases
  local version=$(yq4 e '.vars.version' $varsfile)

  echo "
artefacts:
  - url: https://www.x.org/releases/individual/lib/libX11-${version}.tar.gz
    name: libX11-${version}.tar.gz
" > $f
...
main() {
  ...
  case $mode in
    ...
    set-version)
      read_vars "${source_file}" || return 1
      set_version "${source_file}" "${target_file}" || return 1
      ;;
